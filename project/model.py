...
class Predictor:
    ...
    def predict(self, image: np.ndarray) -> np.ndarray:
        ...
        input_tensor = torch.from_numpy(img_input).unsqueeze(0).float().to(self.device)
        ...
        mask_256 = cv2.erode(mask_256, kernel, iterations=4)
        
        mask_final = cv2.resize(mask_256, (w, h), interpolation=cv2.INTER_NEAREST)
        
        result_mask = (mask_final * 255).astype(np.uint8)
        
        if np.count_nonzero(result_mask) < 950:
